...
static int rzn2_proc_irq_handler(int vect_id, void *data)
{
    unsigned int val = 0U;
...
static int rzn2_enable_interrupt(struct remoteproc *rproc, struct metal_device *ipi_dev)
{
    ...
    if (!ipi.registered) {
        ...
        ret = metal_irq_register((int)irq_vect, rzn2_proc_irq_handler, ipi_dev, rproc);
        if (ret) {
...
rzn2_proc_init(struct remoteproc *rproc,
...
static inline void rzn2_address_translate(metal_phys_addr_t *addr)
{
...
static void *
rzn2_proc_mmap(struct remoteproc *rproc,
            ...
            unsigned int attribute, struct metal_io_region **io)
{
    ...
    rzn2_address_translate(pa);
    rzn2_address_translate(da);
